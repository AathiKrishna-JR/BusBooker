...
export class SeatsComponent implements OnInit {
  ...
  noBusDetails: boolean | undefined;
  totalFare: number = 0;
  selectedSeatNos: number[] = [];
  seatForm: FormGroup | null = null;
...
totalf: number | undefined;
  ...
  constructor(private route: Router,
    private formBuilder: FormBuilder, private get:BusService) {
      ...
      this.bus = Array.isArray(state) ? state : (state ? [state] : []);
  ...
  pay(){
    this.route.navigate(['/pay'],{state:{totalf:this.totalf}});
